...
%>
	...
	<table>
		...
		<tr>
			...
			<g:sortableColumn property="lastLoginDate" title="${message(code: 'user.lastlogin.label', default: 'Last Login Date')}" params="${queryParams}"/>
			<g:sortableColumn property="lastFailedLoginDate" title="${message(code: 'user.lastlogin.faildate.label', default: 'Last Failed Login Date')}" params="${queryParams}"/>
			<g:sortableColumn property="failedLoginCount" title="${message(code: 'user.failedlogin.count.label', default: 'Failed Login Count')}" params="${queryParams}"/>
		</tr>
		</thead>
		...
		<tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
			...
			<td>${fieldValue(bean: user, field: "lastLoginDate")}</td>
			<td>${fieldValue(bean: user, field: "lastFailedLoginDate")}</td>
			<td>${fieldValue(bean: user, field: "failedLoginCount")}</td>
		</tr>
		</g:each>
		</tbody>
	</table>
